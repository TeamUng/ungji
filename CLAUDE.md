...
1. `TODO.md` 확인 → 현재 작업 파악, 완료 항목 확인
2. `git status` + `git branch` → 현재 브랜치·변경사항 파악
3. 작업할 T번호의 **담당 파일 범위** 확인 (다른 T번호 파일 수정 금지)
4. 처음 합류했다면 `AGENTS.md`와 `docs/TEAM_WORKFLOW.md`를 먼저 읽을 것
